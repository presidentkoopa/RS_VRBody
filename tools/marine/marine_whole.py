"""The Doom Eternal Classic Slayer as ONE WHOLE BODY IQM for RS_VRBody.

usage: marine_whole.py MODEL_BASE OUT_DIR [--scale 0.872] [--skin green|blue|red]

The part rig cut this model into a torso, two arms and two hands, each needing its own
wrist socket, stub reshape and pairing. This does not cut anything. One file, the full
152-joint skeleton, bind pose, every chosen bodypart kept whole -- torso, shoulder
plates, arms WITH their hands, legs, pouches, shells, helmet and head.

EACH BODYPART STAYS ITS OWN MESH so the engine can hide one without touching the rest:
the head is the obvious case, since you do not want your own face in your eye.

DROPPED, and why: `hair` (18117 verts of hair cards, more than a third of the model, and
it sits under a helmet), `arm_left_blade` and `torso_grenadelauncher` (equipment the
player is not wearing), `nothing` (a 48-vert placeholder).

FRAME AND UNITS as marine_to_iqm.py: Source's +X left, -Y forward, +Z up, scaled by
SCALE, joint translations scaled with the vertices, winding matched to the Slayer arm.
"""
...
# ---------------------------------------------------------------- the whole body, one file
#
# Order matters only for readability in a mesh list; the engine addresses them by name.
...
whole = []
for bp in KEEP:
    got = bodypart_meshes(bp)
    if not got:
        report.append("WARNING: bodypart %r produced no mesh" % bp)
        continue
    for mi, (mat, tris) in enumerate(got):
        nm = "marine_%s_%s" % (bp, mat.lower())
        whole.append((nm, matpath(mat), tris))

write_iqm(os.path.join(OUT, "marine_whole.iqm"), whole)
report.append("kept bodyparts: %s" % ", ".join(KEEP))
report.append("dropped: %s" % ", ".join(DROP))
open(os.path.join(OUT, "marine_whole_report.txt"), "w").write(chr(10).join(report) + chr(10))
